...
export async function initDb() {
  ...
  await ensureConstitutionDaySubstituteDuty20270716();
...
async function ensureGoldkeyDefaults() {
  const nurses = await queryAll("SELECT id, name FROM users WHERE role IN ('NURSE', 'ANESTHESIA')");
...
/** 2026-11-07~ 마취 당직 순환 재배정 (11/7~8 이지현, 11/14~15 박현정 …) */
async function ensureAnesthesiaDutyFrom2026Nov7() {
  const migrationId = "holiday_duty_anesthesia_2026_nov_v1";
  const done = await queryOne("SELECT id FROM app_migrations WHERE id = ?", migrationId);
  if (done) return;

  const assignFrom = "2026-11-07";
  const endYear = new Date().getFullYear() + 3;
  ...
  for (const row of anesPlan) {
    const hd = String(row.holidayDate ?? "").slice(0, 10);
    const anes = String(row.anesthesiaUserId ?? "").trim();
    if (!hd || !anes) continue;
    ...
    const n1 = String(existing?.nurse1_user_id ?? "").trim();
    const n2 = String(existing?.nurse2_user_id ?? "").trim();

    if (n1 && n2) {
      await execute("UPDATE holiday_duties SET anesthesia_user_id = ? WHERE holiday_date = ?", anes, hd);
      holidayDuties[hd] = { ...holidayDuties[hd], nurse1UserId: n1, nurse2UserId: n2, anesthesiaUserId: anes };
      updated += 1;
      continue;
    }

    const year = Number(hd.slice(0, 4));
    const { plan: orPlan } = buildAutoHolidayDutyPlan({ year, users, holidays, holidayDuties });
    const orRow = orPlan.find((r) => r.holidayDate === hd);
    if (!orRow?.nurse1UserId || !orRow?.nurse2UserId) continue;
    ...
    await execute(
      `INSERT INTO holiday_duties (holiday_date, nurse1_user_id, nurse2_user_id, anesthesia_user_id)
       ...
       ON CONFLICT(holiday_date) DO UPDATE SET
         nurse1_user_id = COALESCE(NULLIF(excluded.nurse1_user_id, ''), holiday_duties.nurse1_user_id),
         nurse2_user_id = COALESCE(NULLIF(excluded.nurse2_user_id, ''), holiday_duties.nurse2_user_id),
         anesthesia_user_id = excluded.anesthesia_user_id`,
      hd,
      orRow.nurse1UserId,
      orRow.nurse2UserId,
      anes
    ...
    holidayDuties[hd] = {
      nurse1UserId: orRow.nurse1UserId,
      nurse2UserId: orRow.nurse2UserId,
      anesthesiaUserId: anes,
    };
    updated += 1;
  }

  await execute("INSERT INTO app_migrations (id) VALUES (?)", migrationId);
  console.log(`[db] anesthesia duty reassigned from ${assignFrom}: ${updated} rows`);
}

/** 2026-12-25~ 당직 순번 재정의(정수영→최종선→…) 및 공휴·주말 재배정 — 11월까지 유지 */
async function ensureOrDutyRotationFrom20261225() {
  const migrationId = "or_duty_rotation_20261225_v1";
  const done = await queryOne("SELECT id FROM app_migrations WHERE id = ?", migrationId);
  if (done) return;

  const assignFrom = OR_DUTY_ROTATION_EFFECTIVE_FROM;
  const endYear = new Date().getFullYear() + 3;
...
/**
 * 주말당직: 11/28·29 임희종 중복 오류 수정 → 장성필·장지은,
 * 이후 순번(12/5·6 정수영·최종선 등) 재배정
 */
async function ensureWeekendDutyFixFrom20261128() {
  const migrationId = "weekend_duty_fix_from_20261128_v1";
  const done = await queryOne("SELECT id FROM app_migrations WHERE id = ?", migrationId);
  if (done) return;

  const assignFrom = "2026-11-28";
  const endYear = new Date().getFullYear() + 3;
  ...
  let upserted = 0;
  const spotCheck = ["2026-11-28", "2026-11-29", "2026-12-05", "2026-12-06"];
  for (const row of plan) {
    const hd = String(row.holidayDate ?? "").slice(0, 10);
    if (!hd || hd < assignFrom) continue;
    ...
    upserted += 1;
    if (spotCheck.includes(hd)) {
      console.log(`[db] ${hd} weekend duty → ${nameById.get(n1)}/${nameById.get(n2)}`);
    }
  }

  // 앵커 강제 보정 (플랜이 비어 있어도 핵심 주말은 맞춤)
  const forced = [
    { dates: ["2026-11-28", "2026-11-29"], n1: "장성필", n2: "장지은" },
    { dates: ["2026-12-05", "2026-12-06"], n1: "정수영", n2: "최종선" },
  ];
  for (const block of forced) {
    const n1 = idByName.get(block.n1) || "";
    const n2 = idByName.get(block.n2) || "";
    if (!n1 || !n2) {
      console.warn(`[db] ${migrationId}: missing users ${block.n1}/${block.n2}`);
      continue;
    }
    for (const hd of block.dates) {
      const existing = await queryOne(
        "SELECT anesthesia_user_id FROM holiday_duties WHERE holiday_date = ?",
        hd
      );
