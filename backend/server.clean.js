...
// 배포 환경에서는 HOST env 오설정(localhost 등) 영향을 받지 않게 항상 공인 바인딩으로 고정.
const HOST = "0.0.0.0";
...
function isDeptHeadRoleServer(role) {
...
/** 수술실 → ADMIN, 마취과 → ADMIN2, 주임 → CHIEF(주임 본인들) */
async function assertActorCanManageLeaveRequest(actorUserId, leaveUserId) {
  ...
  if (leaveRole === "CHIEF") {
    if (actorRole !== "CHIEF") {
      const err = new Error("주임 휴가는 주임 계정으로만 확정·반려할 수 있습니다.");
      err.statusCode = 403;
      throw err;
    }
    return;
  ...
  if (leaveRole === "NURSE") {
    if (!isOrLeaveAdminRoleServer(actorRole)) {
      const err = new Error("수술실 휴가는 관리자·부서파트장만 확정·반려할 수 있습니다.");
      err.statusCode = 403;
      throw err;
    }
    return;
...
app.post("/api/weekly-cell-overrides/sync", async (req, res) => {
  try {
    ...
    const staffRows = await queryAll(
      "SELECT id, role FROM users WHERE role IN ('NURSE', 'ANESTHESIA', 'CHIEF')"
    );
    for (const row of staffRows) {
      staffRoleCache.set(String(row.id), row);
    }

    function staffUserForKey(key) {
      const mk = /^(.+)\|(\d{4}-\d{2}-\d{2})$/.exec(String(key));
      if (!mk) return null;
      return staffRoleCache.get(mk[1]) ?? null;
    }

    const patchedForSideEffects = {};
    await runTransaction(async (tx) => {
      for (const [key, raw] of incomingEntries) {
        const staff = staffUserForKey(key);
        ...
        const ymd = mk[2];
        const val = raw && typeof raw === "object" ? raw : null;
        await tx.execute("DELETE FROM weekly_cell_overrides WHERE user_id = ? AND ymd = ?", userId, ymd);
        if (val && String(val.mode ?? "") === "manual") {
          ...
          patchedForSideEffects[key] = { mode: "manual", kind, main, sub };
        }
      }
    });

    await applyWeeklyOverridesToSubstitutes(patchedForSideEffects, actorUserId);
    await applyWeeklyLeaveOverridesToRequestNature(patchedForSideEffects);
    return res.json({ ok: true, patched: Object.keys(patchedForSideEffects).length });
  } catch (err) {
...
app.post("/api/emergency-surgery/notify", async (req, res) => {
  try {
    ...
    const fromDeptHead = actor && isDeptHeadRoleServer(actor.role);
    if (!fromEmergencyOr && !fromDeptHead) {
      return res.status(403).json({ error: "의국·부서파트장 계정만 응급수술 알림을 보낼 수 있습니다." });
    }
...
const REGISTRATION_APPROVE_ROLES = new Set(["NURSE", "ANESTHESIA", "CHIEF", "DEPT_HEAD"]);
  ...
  try {
    ...
    if (!REGISTRATION_APPROVE_ROLES.has(role)) {
      return res.status(400).json({
        error: "역할은 수술실 간호사, 마취과 간호사, 주임, 부서파트장 중 하나여야 합니다.",
      });
    }
...
async function canActorUseHolidayDutyDayMemoForDate(actor, ymd) {
  ...
  if (isDeptHeadRoleServer(role)) return true;
  const duty = await queryOne(
    "SELECT nurse1_user_id, nurse2_user_id, anesthesia_user_id FROM holiday_duties WHERE holiday_date = ?",
    ymd
  );
...
async function canManageHolidayDutyDayCommentServer(actor, commentUserId) {
  ...
  return isDeptHeadRoleServer(actor?.role);
}
  ...
  try {
    ...
    if (offDay) {
      if (!(await canActorUseHolidayDutyDayMemoForDate(actor, ymd))) {
        return res.status(403).json({ error: "휴일 메모는 당직 간호사·부서파트장·의국만 작성할 수 있습니다." });
      }
    } else if (!["ADMIN", "DEPT_HEAD", "NURSE", "ANESTHESIA"].includes(String(actor.role))) {
...
app.post("/api/day-comments/:id/update", async (req, res) => {
  try {
    ...
    const canManage = offDay
      ...
      : isOrLeaveAdminRoleServer(actor.role) || row.user_id === actorUserId;
...
app.post("/api/notice-comments", async (req, res) => {
  try {
    ...
    if (!nid) return res.status(400).json({ error: "공지 id가 필요합니다." });
    const noticeRow = await queryOne("SELECT id FROM notices WHERE id = ?", nid);
    if (!noticeRow) return res.status(404).json({ error: "공지사항을 찾을 수 없습니다." });
    const txt = String(content ?? "").trim();
    if (!txt) return res.status(400).json({ error: "댓글 내용을 입력하세요." });
    if (txt.length > 500) return res.status(400).json({ error: "댓글은 500자 이내로 입력하세요." });
    const nowIso = new Date().toISOString();
    const id = `nc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await execute(
      "INSERT INTO notice_comments (id, notice_id, user_id, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
      id,
      nid,
      actorUserId,
      txt,
    ...
    console.error("POST /api/notice-comments", err);
    return res.status(500).json({ error: String(err?.message || err) });
  }
});

app.post("/api/notice-comments/:id/update", async (req, res) => {
  try {
    const commentId = String(req.params.id ?? "").trim();
    if (!commentId) return res.status(400).json({ error: "댓글 id가 필요합니다." });
    const { actorUserId, content } = req.body ?? {};
    const txt = String(content ?? "").trim();
    if (!txt) return res.status(400).json({ error: "댓글 내용을 입력하세요." });
    if (txt.length > 500) return res.status(400).json({ error: "댓글은 500자 이내로 입력하세요." });
    ...
    const actor = await queryOne("SELECT id, role FROM users WHERE id = ?", actorUserId);
    if (!actor) return res.status(403).json({ error: "권한이 없습니다." });
    const row = await queryOne("SELECT id, user_id FROM notice_comments WHERE id = ?", commentId);
    if (!row) return res.status(404).json({ error: "댓글을 찾을 수 없습니다." });

    const canManage = isOrLeaveAdminRoleServer(actor.role) || String(row.user_id) === String(actorUserId);
    if (!canManage) return res.status(403).json({ error: "작성자 또는 관리자만 수정할 수 있습니다." });

    await execute(
      "UPDATE notice_comments SET content = ?, updated_at = ? WHERE id = ?",
      txt,
      new Date().toISOString(),
      commentId
    );
    ...
    console.error("POST /api/notice-comments/:id/update", err);
    return res.status(500).json({ error: String(err?.message || err) });
  }
});

app.post("/api/notice-comments/:id/delete", async (req, res) => {
  try {
    const commentId = String(req.params.id ?? "").trim();
    if (!commentId) return res.status(400).json({ error: "댓글 id가 필요합니다." });
    const { actorUserId } = req.body ?? {};
    ...
    await execute("DELETE FROM notice_comments WHERE id = ?", commentId);
    return res.json({ ok: true });
  } catch (err) {
    console.error("POST /api/notice-comments/:id/delete", err);
    return res.status(500).json({ error: String(err?.message || err) });
  }
});
...
app.post("/api/admin/holiday-duties", async (req, res) => {
  try {
    ...
    const actor = await queryOne(
      "SELECT id, role FROM users WHERE id = ? AND role IN ('ADMIN', 'DEPT_HEAD', 'NURSE', 'ANESTHESIA')",
      actorUserId
    );
    if (!actor) return res.status(403).json({ error: "권한이 없습니다." });
...
app.post("/api/substitute-assignments/:requestId/upsert", async (req, res) => {
  try {
    ...
    if (standaloneNurseM || standaloneAnesM || standaloneChiefM) {
      ...
      } else if (!isOrLeaveAdminRoleServer(actor.role)) {
        return res.status(403).json({ error: "수술실 대체 근무는 관리자·부서파트장만 지정할 수 있습니다." });
      }
    } else {
      ...
      if (leaveRole === "ANESTHESIA") {
        ...
        if (actor.role !== "CHIEF") {
          return res.status(403).json({ error: "주임 대체 근무는 주임 계정으로만 지정할 수 있습니다." });
        }
...
app.post("/api/requests/:id/uncancel", async (req, res) => {
  try {
    ...
    if (
      !actor ||
      (actor.role !== "ADMIN" && actor.role !== "DEPT_HEAD" && actor.role !== "ADMIN2" && actor.role !== "CHIEF")
    ) {
...
app.post("/api/admin/goldkeys/usage-bulk", async (req, res) => {
  try {
    ...
    const actor = await queryOne(
      "SELECT id FROM users WHERE id = ? AND role IN ('ADMIN', 'DEPT_HEAD', 'NURSE')",
      actorUserId
    );
    if (!actor) return res.status(403).json({ error: "권한이 없습니다." });
