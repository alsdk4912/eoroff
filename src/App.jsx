...
function App() {
  ...
  async function updateLeaveNatureForRequest(requestId, nextNature) {
    ...
    setRequests((prev) => {
      const next = prev.map((r) => (r.id === requestId ? { ...r, leaveNature: n } : r));
      try {
        localStorage.setItem(LS_REQUESTS, JSON.stringify(next));
      } catch {
        /* ignore */
...
function RequestPage({
  ...
  return (
    <section className="card">
      ...
      <p className="help page-lead">날짜와 구분을 고른 뒤 제출합니다. 캘린더에서도 같은 신청을 할 수 있습니다.</p>
...
function MyRequestsPage({ myRequests, cancelRequest, uncancelRequest, canUncancel, serverMode, onUpdateLeaveNature }) {
  const [yearFilter, setYearFilter] = useState("ALL");
  const [leaveTypeFilter, setLeaveTypeFilter] = useState("ALL");
  const [leaveMarkFilter, setLeaveMarkFilter] = useState("ALL");
  ...
  function matchesLeaveMarkFilter(r) {
    if (leaveMarkFilter === "ALL") return true;
    return String(r.leaveNature ?? "PERSONAL") === leaveMarkFilter;
  }
...
const WORK_SCHEDULE_2026_ROWS = [
  ...
  { name: "장지은", values: ["1D2", "6D1", "6D1", "6D2", "5D1", "7D2", "안D0", "안D0", "PRN", "수E", "9-5", "1D1"] },
...
const SEPT_2026_SCHEDULE_FIXES = new Map([
  ["이현숙", { from: "7D2", to: "수E" }],
  ["장지은", { from: "수E", to: "PRN" }],
  ["최종선", { from: "7D1", to: "7D2" }],
  ["장성필", { from: "5D1", to: "7D1" }],
      ...
      {dashTab === "summary" && (currentRole === "NURSE" || currentRole === "ADMIN" || currentRole === "DEPT_HEAD" || currentRole === "ANESTHESIA" || currentRole === "ADMIN2") ? (
        <section className="card">
          ...
          <p className="help page-lead goldkey-policy-note">
            <span>선택한 연도(1/1~12/31) 휴가일 기준 집계 · 신청 시 차감</span>
            <span>장기휴가 모집: 상반기←전년 10/1~10 · 하반기←당해 4/1~10 (기간 내 취소만 차감 면제)</span>
          </p>
          <div className="table-wrap">
            <table>
              <thead>
